...
return Array.isArray(text) ? text.map(chunk => 'text' in chunk ? chunk.text : '').join('') : text;    } catch (error: any) {
...
export async function summariseCode(doc: Document) {
    ...
    try {
        ...
        // Handle both string and ContentChunk[] types
        return Array.isArray(text) ? text.map(chunk => 'text' in chunk ? chunk.text : '').join('') : text;
    } catch (error: any) {
